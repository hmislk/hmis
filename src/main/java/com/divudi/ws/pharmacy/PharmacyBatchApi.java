...
/**
 * REST API for Pharmacy Batch Management
 * Provides endpoints for AMP search/creation and batch creation with Stock entries
 *
 * @author Buddhika
 */
@Path("pharmacy_batches")
@RequestScoped
public class PharmacyBatchApi {
     ...
     */
    @POST
    @Path("/amp/search_or_create")
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response searchOrCreateAmp(String requestBody) {
        try {
            // Validate API key
            ...
            // Parse request body
            AmpSearchCreateRequestDTO request;
            try {
                request = gson.fromJson(requestBody, AmpSearchCreateRequestDTO.class);
            ...
            // Process search or create
            AmpResponseDTO response = batchService.searchOrCreateAmp(request, user);
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response createBatch(String requestBody) {
        try {
            // Validate API key
            String key = requestContext.getHeader("Finance");
            WebUser user = validateApiKey(key);
    ...
    /**
     * Search AMPs by name
     * Endpoint: GET /api/pharmacy_batches/amp/search?name=ItemName&limit=30
     */
    @GET
    @Path("/amp/search")
    @Produces(MediaType.APPLICATION_JSON)
    public Response searchAmp() {
        try {
            // Validate API key
            String key = requestContext.getHeader("Finance");
            WebUser user = validateApiKey(key);
            ...
            if (name == null || name.trim().isEmpty()) {
                return errorResponse("Query parameter 'name' is required", 400);
            }

            // Parse limit
            Integer limit = 30; // Default limit
            if (limitStr != null && !limitStr.trim().isEmpty()) {
                try {
                    limit = Integer.parseInt(limitStr);
                } catch (NumberFormatException e) {
                    return errorResponse("Invalid limit parameter: must be a number", 400);
                }
            }

            // Process search
            List<AmpResponseDTO> response = batchService.searchAmpByName(name, limit);
            return successResponse(response);
     ...
     * Following PharmacyAdjustmentApi pattern
     */
    ...
    /**
     * Create error response following existing pattern
     */
