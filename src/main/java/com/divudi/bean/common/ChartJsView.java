...
public class ChartJsView implements Serializable {
    ...
    private LineChartModel countChartModel;
    private LineChartModel serviceChartModel;

    @PostConstruct
    public void init() {
        createPieModel();
        createPolarAreaModel();
        createLineModelForRouteAnalysisReport();
    ...
    public void createLineModelForRouteAnalysisReport() {
    ...
    public void createLineModelForRouteAnalysisReport(final Map<YearMonth, Double> chartData, final String titleText,
                                                      final String yAxisLabel) {
        lineModel = new LineChartModel();
        ChartData data = new ChartData();

        LineChartDataSet dataSet = new LineChartDataSet();
        List<Object> values = new ArrayList<>(chartData.values());

        dataSet.setData(values);
        dataSet.setFill(false);
        dataSet.setLabel(yAxisLabel);
        dataSet.setBorderColor("rgb(75, 192, 192)");
        dataSet.setTension(0.1);
        data.addChartDataSet(dataSet);

        List<String> labels = chartData.keySet().stream()
    ...
    public LineChartModel getCountChartModel() {
        if (countChartModel == null) {
            createCountChartModel();
        }
        return countChartModel;
    }

    public LineChartModel getServiceChartModel() {
        if (serviceChartModel == null) {
            createServiceChartModel();
        }
        return serviceChartModel;
    }

    public void createCountChartModel() {
        createLineModelForRouteAnalysisReport(reportsController.getSampleCountChartData(), "Count vs Month", "Sample Count");
        countChartModel = lineModel;
    }

    public void createServiceChartModel() {
        createLineModelForRouteAnalysisReport(reportsController.getServiceAmountChartData(), "Total vs Month", "Service Amount");
        serviceChartModel = lineModel;
    }
