...
/**
 * Controller for Patient-Specific Deposit History Report.
 * Shows deposit histories for a selected patient with optional filters.
 * Patient must be selected first before processing the report.
 *
 * @author Dr M H B Ariyaratne
 */
@Named
@SessionScoped
public class PatientSpecificDepositHistoryReportController implements Serializable {
    ...
    public PatientSpecificDepositHistoryReportController() {
    }
    ...
    /**
     * Generate the Patient-Specific Deposit History report.
     * Requires patient to be selected first.
     */
    public void processPatientDepositHistoriesReport() {
        ...
        initializeDates();
        patientDepositHistoryDtos = fetchPatientSpecificDepositHistoryDtos();
        calculateTotals();

        if (patientDepositHistoryDtos == null || patientDepositHistoryDtos.isEmpty()) {
            JsfUtil.addErrorMessage("No deposit history found for the selected patient");
        }
    }

    /**
     * Fetch patient deposit histories for a specific patient as DTOs.
     * Includes filters for date range, institution, site, and department.
     */
    private List<PatientDepositHistoryDto> fetchPatientSpecificDepositHistoryDtos() {
     ...
     */
    public void clearPatientDepositHistoriesReport() {
        fromDate = CommonFunctions.getStartOfDay(new Date());
        toDate = CommonFunctions.getEndOfDay(new Date());
        institution = null;
        site = null;
    ...
    /**
     * Navigate to the patient-specific histories report page.
     */
    public String navigateToPatientSpecificDepositHistoriesReport() {
        clearPatientDepositHistoriesReport();
        return "/patient_deposit/patient_specific_deposit_histories_report?faces-redirect=true";
    }
