...
//        }
    ...
    public void processOpdServiceCountDoctorWise() {
        String jpql = "select new com.divudi.data.ItemCount(bi.bill.referredBy.person.name, bi.item.name, count(bi.item)) "
                + " from BillItem bi "
                + " where bi.bill.cancelled=:can "
                + " and bi.bill.billDate between :fd and :td ";
        ...
        Map<String, CategoryCount> doctorWiseReport = new HashMap<>();
        
        for (ItemCount count : reportOpdServiceCount) {
            doctorWiseReport.computeIfAbsent(count.getCategory(), k -> new CategoryCount(k, new ArrayList<>(), 0L))
                    .getItems().add(count);
            doctorWiseReport.get(count.getCategory()).setTotal(doctorWiseReport.get(count.getCategory()).getTotal() + count.getTestCount());
        }

        // Convert the map values to a list to be used in the JSF page
        reportList = new ArrayList<>(doctorWiseReport.values());
