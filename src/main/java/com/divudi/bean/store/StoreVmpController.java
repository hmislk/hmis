...
@Named
@SessionScoped
public class StoreVmpController implements Serializable {
        ...
        try {
            if (getCurrent().getId() != null && getCurrent().getId() > 0) {
                ...
                auditService.logAudit(beforeData, afterData,
                        ...
                        "StoreVmp", "Update Store VMP", getCurrent().getId());
                ...
                auditService.logAudit(null, afterData,
                        getSessionController().getLoggedUser(),
                        "StoreVmp", "Create Store VMP", getCurrent().getId());

                JsfUtil.addSuccessMessage("Saved Successfully.");
            }
            editable = false;
        } catch (Exception e) {
            JsfUtil.addErrorMessage("Error saving Store VMP: " + e.getMessage());
            e.printStackTrace();
        }
    ...
    public void delete() {
        if (current != null) {
            try {
                ...
                auditService.logAudit(beforeData, afterData,
                        getSessionController().getLoggedUser(),
                        "StoreVmp", "Delete Store VMP", current.getId());

                JsfUtil.addSuccessMessage("Deleted Successfully");
            } catch (Exception e) {
                JsfUtil.addErrorMessage("Error deleting Store VMP: " + e.getMessage());
                e.printStackTrace();
            }
        } else {
    ...
    public String getFilterStatusDisplay() {
        switch (filterStatus) {
            ...
            case "inactive":
                return "Inactive Store VMPs";
            case "all":
                return "All Store VMPs";
            default:
                return "Active Store VMPs";
        }
    }
    ...
    public void toggleVmpStatus() {
        ...
        if (wasInactive) {
            ...
            JsfUtil.addSuccessMessage("Store VMP Activated Successfully");
        } else {
            current.setInactive(true);
            JsfUtil.addSuccessMessage("Store VMP Deactivated Successfully");
        }

        getFacade().edit(current);

        Map<String, Object> afterData = createAuditMap(current);
        String action = wasInactive ? "Activate Store VMP" : "Deactivate Store VMP";
        auditService.logAudit(beforeData, afterData,
                getSessionController().getLoggedUser(),
                "StoreVmp", action, current.getId());
    ...
    public void fillVmpAuditEvents() {
        ...
        if (current != null && current.getId() != null) {
            try {
                ...
                params.put("entityType", "StoreVmp");
    ...
    public String navigateToVmpAuditEvents() {
        ...
        fillVmpAuditEvents();
        return "/pharmacy/admin/store_vmp_audit_events?faces-redirect=true";
    }
    ...
    @FacesConverter("storeVmpDtoConverter")
    public static class StoreVmpDtoConverter implements Converter {
        ...
        public Object getAsObject(FacesContext facesContext, UIComponent component, String value) {
            ...
            try {
                ...
                StoreVmpController controller = (StoreVmpController) facesContext.getApplication()
                        .getELResolver().getValue(facesContext.getELContext(), null, "storeVmpController");
