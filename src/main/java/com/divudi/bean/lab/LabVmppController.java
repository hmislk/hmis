...
@Named
@SessionScoped
public class LabVmppController implements Serializable {
    ...
    private List<VmppDto> labVmppListDtos;
        ...
        try {
            if (getCurrent().getId() != null && getCurrent().getId() > 0) {
                ...
                auditService.logAudit(beforeEditData, afterData,
                        ...
                        "LabVmpp", "Update Lab VMPP", getCurrent().getId());
                ...
                auditService.logAudit(null, afterData,
                        getSessionController().getLoggedUser(),
                        "LabVmpp", "Create Lab VMPP", getCurrent().getId());
        ...
        if (current != null) {
            try {
                ...
                auditService.logAudit(beforeData, afterData,
                        getSessionController().getLoggedUser(),
                        "LabVmpp", "Delete Lab VMPP", current.getId());

                JsfUtil.addSuccessMessage("Deleted Successfully");
            } catch (Exception e) {
                JsfUtil.addErrorMessage("Error deleting Lab VMPP: " + e.getMessage());
                e.printStackTrace();
            }
        } else {
    ...
    public String getFilterStatusDisplay() {
        switch (filterStatus) {
            case "active":
                return "Active Lab VMPPs";
            case "inactive":
                return "Inactive Lab VMPPs";
            case "all":
                return "All Lab VMPPs";
            default:
                return "Active Lab VMPPs";
        }
    }
    ...
    public List<VmppDto> getLabVmppListDtos() {
        if (labVmppListDtos == null) {
            String jpql = "SELECT new com.divudi.core.data.dto.VmppDto("
                    + "a.id, a.name, a.code, "
            ...
            labVmppListDtos = (List<VmppDto>) getFacade().findLightsByJpql(jpql, params);
        }
        return labVmppListDtos;
    }

    public String navigateToLabVmppList() {
        labVmppListDtos = null;
        getLabVmppListDtos();
        return "/pharmacy/admin/lab_vmpp_list?faces-redirect=true";
    }
    ...
    public void toggleVmppStatus() {
        ...
        if (wasInactive) {
            ...
            JsfUtil.addSuccessMessage("Lab VMPP Activated Successfully");
        } else {
            current.setInactive(true);
            JsfUtil.addSuccessMessage("Lab VMPP Deactivated Successfully");
        }

        getFacade().edit(current);

        Map<String, Object> afterData = createAuditMap(current);
        String action = wasInactive ? "Activate Lab VMPP" : "Deactivate Lab VMPP";
        auditService.logAudit(beforeData, afterData,
                getSessionController().getLoggedUser(),
                "LabVmpp", action, current.getId());
    ...
    public String navigateToVmppAuditEvents() {
        ...
        fillVmppAuditEvents();
        return "/pharmacy/admin/lab_vmpp_audit_events?faces-redirect=true";
    }
    ...
    @FacesConverter("labVmppDtoConverter")
    public static class LabVmppDtoConverter implements Converter {
        ...
        public Object getAsObject(FacesContext facesContext, UIComponent component, String value) {
            ...
            try {
                ...
                LabVmppController controller = (LabVmppController) facesContext.getApplication()
                        .getELResolver().getValue(facesContext.getELContext(), null, "labVmppController");
