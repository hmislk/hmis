...
@Named
@SessionScoped
public class LabVtmController implements Serializable {
        ...
        try {
            ...
            if (getCurrent().getId() != null) {
                ...
                auditService.logAudit(beforeData, afterData,
                        ...
                        "LabVtm", "Update Lab VTM", getCurrent().getId());

                JsfUtil.addSuccessMessage("Lab VTM '" + getCurrent().getName() + "' updated successfully");
            } else {
                // CREATE
                ...
                auditService.logAudit(null, afterData,
                        getSessionController().getLoggedUser(),
                        "LabVtm", "Create Lab VTM", getCurrent().getId());

                JsfUtil.addSuccessMessage("Lab VTM '" + getCurrent().getName() + "' created successfully");
            ...
            JsfUtil.addErrorMessage("Error saving Lab VTM '" + (getCurrent().getName() != null ? getCurrent().getName() : "Unknown") + "': " + e.getMessage());
            e.printStackTrace();
        }
    ...
    public void delete() {
        if (current != null) {
            ...
            try {
                ...
                auditService.logAudit(beforeData, afterData,
                        getSessionController().getLoggedUser(),
                        "LabVtm", "Delete Lab VTM", current.getId());

                JsfUtil.addSuccessMessage("Lab VTM '" + (vtmName != null ? vtmName : "Unknown") + "' deleted successfully");
            } catch (Exception e) {
                JsfUtil.addErrorMessage("Error deleting Lab VTM '" + (vtmName != null ? vtmName : "Unknown") + "': " + e.getMessage());
                e.printStackTrace();
            }
        } else {
    ...
    public List<VtmDto> getVtmDtos() {
        if (vtmDtoList == null) {
            String jpql = "SELECT new com.divudi.core.data.dto.VtmDto("
                    ...
                    + "v.retired, "
                    + "v.inactive, "
                    + "COALESCE(CAST(v.departmentType AS string), 'Lab')) "
    ...
    public void toggleVtmStatus() {
        ...
        if (wasInactive) {
            ...
            JsfUtil.addSuccessMessage("Lab VTM Activated Successfully");
        } else {
            current.setInactive(true);
            JsfUtil.addSuccessMessage("Lab VTM Deactivated Successfully");
        }

        getFacade().edit(current);

        Map<String, Object> afterData = createAuditMap(current);
        String action = wasInactive ? "Activate Lab VTM" : "Deactivate Lab VTM";
        auditService.logAudit(beforeData, afterData,
                getSessionController().getLoggedUser(),
                "LabVtm", action, current.getId());
    ...
    public String navigateToAuditEvents() {
        ...
        fillVtmAuditEvents();
        return "/pharmacy/admin/lab_vtm_audit_events?faces-redirect=true";
    }
    ...
    @FacesConverter("labVtmDtoConverter")
    public static class LabVtmDtoConverter implements Converter {
        ...
        public Object getAsObject(FacesContext facesContext, UIComponent component, String value) {
            ...
            try {
                ...
                LabVtmController controller = (LabVtmController) facesContext.getApplication()
                        .getELResolver().getValue(facesContext.getELContext(), null, "labVtmController");
