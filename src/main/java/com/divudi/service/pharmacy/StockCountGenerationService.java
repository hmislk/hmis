    ...
    public void generateStockCountBillAsync(String jobId, Department department,
        ...
        try {
            ...
            for (StockDTO dto : stockDTOs) {
                ...
                // Use plain new objects with only ID set - zero DB queries, works with native SQL persist
            ...
            System.out.println("[StockCountGen] Loop complete. items=" + processed + " ms=" + (System.currentTimeMillis() - tLoop));
            ...
            if (includeZeroStock && zeroStockLimit > 0) {
                ...
                if (zeroStockDTOs != null && !zeroStockDTOs.isEmpty()) {
                    LOGGER.log(Level.INFO, "[StockCountGen] Processing {0} zero-stock items...", zeroStockDTOs.size());
