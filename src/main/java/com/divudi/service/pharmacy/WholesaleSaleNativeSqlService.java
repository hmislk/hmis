...
/**
 * Persists pharmacy wholesale sale bills using native SQL for the hot-path
 * operations (stock deduction, aggregates, StockHistory INSERT, Payment INSERT).
 *
 * Duplicated from RetailSaleNativeSqlService, re-pointed so that the value
 * charged/recorded as "sold at" uses the batch wholesaleRate instead of the
 * batch retailRate (both are still written for every line, for bookkeeping
 * symmetry, as in the retail service).
 *
 ...
 */
@Stateless
public class WholesaleSaleNativeSqlService {

    private static final Logger LOGGER = Logger.getLogger(WholesaleSaleNativeSqlService.class.getName());
    ...
    public List<Payment> settle(PreBill preBill, BilledBill saleBill, List<BillItemData> items,
        ...
        LOGGER.log(Level.INFO, "[WholesaleNativeSettle] PreBill id={0} BilledBill id={1} ms={2}",
        ...
        LOGGER.log(Level.INFO, "[WholesaleNativeSettle] BillItem+PBI inserted ms={0}", System.currentTimeMillis() - t0);
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            StockAggregateResult agg = computeAggregates(
                    ...
                    postDeductQty,
                    item.getBatchWholesaleRate(), item.getBatchPurchaseRate(),
        ...
        LOGGER.log(Level.INFO, "[WholesaleNativeSettle] Stock deducted + history inserted ms={0}", System.currentTimeMillis() - t0);
    ...
    /**
     * Loads view data for an existing settled wholesale sale bill.
     * Returns Object[]{PrintBillData, List&lt;BillItemData&gt;}.
     * Used by WholesaleSaleNativeSqlController.viewByBillId to display
