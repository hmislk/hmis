...
/**
 * Print DTO for one item line in a transfer receive note.
 * Populated by TransferReceiveNativeSqlService.settle() and
 * rendered by the DTO-based composite print components.
 */
public class TransferReceiveItemPrintDto implements Serializable {
    ...
    /** Received quantity in packs. */
    private double qty;

    /** Received quantity in units (qty * unitsPerPack). */
    private double qtyInUnits;

    /** Original issued quantity in units (for "Issued QTY" column). */
    private double issuedQty;

    private double rate;
    private double netRate;

    /** Stored as negative (receive = money going out); display with abs(). */
    ...
    /** Line gross total (negative — receive direction). */
    private BigDecimal lineGrossTotal;

    public TransferReceiveItemPrintDto() {
    }
    ...
    public void setQtyInUnits(double qtyInUnits) {
