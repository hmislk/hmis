...
BEGIN
    ...
    -- Rollback other columns to their v2.1.1 types
    -- Note: Restoring to DECIMAL(18,4) to match prior migration (v2.1.1)
    SELECT 'Rolling back other financial columns to DECIMAL(18,4)...' AS progress;

    ALTER TABLE BILLFINANCEDETAILS MODIFY COLUMN TOTALPURCHASEVALUE DECIMAL(18,4);
    SELECT '✓ TOTALPURCHASEVALUE rolled back to DECIMAL(18,4)' AS result;

    ALTER TABLE BILLFINANCEDETAILS MODIFY COLUMN TOTALRETAILSALEVALUE DECIMAL(18,4);
    SELECT '✓ TOTALRETAILSALEVALUE rolled back to DECIMAL(18,4)' AS result;

    ALTER TABLE BILLFINANCEDETAILS MODIFY COLUMN TOTALDISCOUNT DECIMAL(18,4);
    SELECT '✓ TOTALDISCOUNT rolled back to DECIMAL(18,4)' AS result;

    ALTER TABLE BILLFINANCEDETAILS MODIFY COLUMN BILLEXPENSE DECIMAL(18,4);
    SELECT '✓ BILLEXPENSE rolled back to DECIMAL(18,4)' AS result;

    ALTER TABLE BILLFINANCEDETAILS MODIFY COLUMN LINEEXPENSE DECIMAL(18,4);
    SELECT '✓ LINEEXPENSE rolled back to DECIMAL(18,4)' AS result;
