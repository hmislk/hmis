...
SET @sql = IF(@col_exists > 0,
    'SELECT "SKIPPED: ARCHIVEDAT already present on ITEMBATCHARCHIVE" AS status',
    'ALTER TABLE ITEMBATCHARCHIVE MODIFY COLUMN ID BIGINT NOT NULL, ADD COLUMN ARCHIVEDAT DATETIME NULL'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = IF(@col_exists > 0,
    'SELECT "SKIPPED: ARCHIVEDAT already present on STOCKARCHIVE" AS status',
    'ALTER TABLE STOCKARCHIVE MODIFY COLUMN ID BIGINT NOT NULL, ADD COLUMN ARCHIVEDAT DATETIME NULL'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
