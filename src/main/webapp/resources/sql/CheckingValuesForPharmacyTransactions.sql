...
SELECT id, `CREATEDAT`, `BILLITEMFINANCEDETAILS_ID`, `NETRATE`, `RATE`, `QTY`, `NETVALUE`, `GROSSVALUE` FROM billitem order by id desc LIMIT 2;
...
select `ID`, `QTY`, `RETAILRATE`, `PURCHASERATE`, `COSTRATE` ,  `PURCHASERATEPACK`, `RETAILRATEPACK`, `COSTRATEPACK`, `PURCHASEVALUE`, `RETAILVALUE`, `COSTVALUE`,  `STOCK_ID`, `ITEMBATCH_ID` from pharmaceuticalbillitem order by id desc limit 2;

-- Check the latest bill header information
-- This query retrieves the most recent bill with key totals and transaction type
select `ID` , `CREATEDAT`, `BILLTYPEATOMIC`,  `NETTOTAL`, `TOTAL`, `SALEVALUE` from Bill order by id desc limit 2;

-- Check the latest bill finance details summary
-- This query fetches bill-level financial totals for verification
select `ID`, `NETTOTAL`, `GROSSTOTAL`, `TOTALCOSTVALUE`, `TOTALPURCHASEVALUE`, `TOTALRETAILSALEVALUE` from billfinancedetails order by id desc limit 2;